...
export default {
  ...
  methods:
    {
      ...
      {
        axios
        .delete(`https://localhost:7216/api/Users/${id}`)
         // console.log(id);
         location.reload();  
      },
  ...
  preConfirm: () => {
    ...
    axios.post(`https://localhost:7216/api/Users`,u);
    location.reload(); 
  }
});
if (formValues) {
  ...
  preConfirm: () => {
    ...
    axios.put(`https://localhost:7216/api/Users?id=${u.id}`,u);
    location.reload(); 
  }
});
if (formValues) {
  ...
  created()
    {
      ...
      axios
      .get('https://localhost:7216/api/Users')
      .then((response) => {
